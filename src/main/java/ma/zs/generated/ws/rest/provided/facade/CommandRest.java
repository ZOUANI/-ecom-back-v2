...
public class CommandRest {
	...
	@ApiOperation("Finds command by id of deliveryCost")
	@GetMapping("/deliveryCost/id/{id}")
	public List<CommandVo> findByDeliveryCostId(@PathVariable Long id){
		return commandConverter.toVo(commandService.findByDeliveryCostId(id));
	}
	@ApiOperation("Deletes command by id of deliveryCost")
	@DeleteMapping("/deliveryCost/id/{id}")
	public int deleteByDeliveryCostId(@PathVariable Long id){
		return commandService.deleteByDeliveryCostId(id);
	}
