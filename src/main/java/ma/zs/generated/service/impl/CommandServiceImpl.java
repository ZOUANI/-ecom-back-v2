...
public class CommandServiceImpl implements CommandService {
   ...
   private EntityManager entityManager; 
    ...
    public int deleteByDeliveryId(Long id){
     ...
     @Override
    public List<Command> findByDeliveryCostId(Long id){
		return commandDao.findByDeliveryCostId(id);

	}
	   @Override
	   @Transactional
    public int deleteByDeliveryCostId(Long id){
		return commandDao.deleteByDeliveryCostId(id);
   ...
   public void deleteById(Long id){
	...
	public Command save (Command command){
	       ...
	       if(foundedCommand!=null)
	          ...
	          if(command.getDeliveryCost()!=null){
                      DeliveryCost deliveryCost = deliveryCostService.findById(command.getDeliveryCost().getId());
				  
				  if(deliveryCost == null)
				  command.setDeliveryCost(deliveryCostService.save(command.getDeliveryCost()));
				  else
				  command.setDeliveryCost(deliveryCost);
			  }
	    ...
	    Command savedCommand = commandDao.save(command);
		  ...
		  savedCommand.setComments(commentService.save(prepareComments(savedCommand,command.getComments())));
               ...
               if(ListUtil.isNotEmpty(command.getCommandeAccesses())){
		  savedCommand.setCommandeAccesses(commandeAccessService.save(prepareCommandeAccesses(savedCommand,command.getCommandeAccesses())));
		 }
	   return savedCommand;
	}
    ...
    public List<Command> save(List<Command> commands){
     ...
     private List<CommandeAccess> prepareCommandeAccesses(Command command,List<CommandeAccess> commandeAccesss){
		    for(CommandeAccess commandeAccess:commandeAccesss ){
                  commandeAccess.setCommand(command); 
			}
			return commandeAccesss;
	  }
   ...
   if(commandVo.getDeliveryCostVo()!=null){
     query += SearchUtil.addConstraint( "o", "deliveryCost.id","=",commandVo.getDeliveryCostVo().getId());
   }
